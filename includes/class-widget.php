...
class TrolyWP_Agent_Client_Widget extends WP_Widget {
    ...
    public function widget($args, $instance) {
        // Không làm gì ở đây, sẽ inject widget qua hook riêng để luôn hiển thị cho admin ở mọi trang.
    }
}

// Inject widget chat cho admin ở mọi trang (frontend/backend)
add_action('wp_footer', function() {
    ...
    echo '<div id="trolywp-chat-panel" style="display:none;position:fixed;top:0;right:0;width:350px;height:100vh;background:#fff;border-radius:12px 0 0 12px;box-shadow:-2px 0 16px rgba(0,0,0,0.2);z-index:9999;overflow:hidden;transition:right 0.3s;">
        <div id="trolywp-chat-resizer" style="position:absolute;left:0;top:0;width:6px;height:100%;cursor:ew-resize;z-index:10001;background:rgba(0,0,0,0.03);"></div>
    ...
    var resizer = document.getElementById("trolywp-chat-resizer");
    var currentMode = "sidebar";
    // Lấy trạng thái từ localStorage khi mở panel
    var savedMode = localStorage.getItem("trolywp_chat_mode");
    var savedWidth = localStorage.getItem("trolywp_chat_width");
    if (savedMode) currentMode = savedMode;
    function setMode(mode) {
        currentMode = mode;
        localStorage.setItem("trolywp_chat_mode", mode);
        if (mode === "sidebar") {
            panel.style.width = savedWidth ? savedWidth : "350px";
    ...
    // Kéo co giãn chiều rộng sidebar
    ...
    document.addEventListener("mouseup", function(e) {
        if (isResizing) {
            isResizing = false;
            localStorage.setItem("trolywp_chat_width", panel.style.width);
