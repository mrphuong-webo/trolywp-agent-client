    ...
    public function widget($args, $instance) {
        ...
        if (empty($webhook_url)) {
            ...
            echo '<div id="trolywp-chat-panel" style="display:none;position:fixed;top:0;right:0;width:350px;height:100vh;background:#fff;border-radius:12px 0 0 12px;box-shadow:-2px 0 16px rgba(0,0,0,0.2);z-index:9999;overflow:hidden;transition:right 0.3s;">
                <div style="position:absolute;top:8px;left:8px;z-index:10000;display:flex;gap:8px;">
                    <button id="trolywp-chat-close" style="background:#eee;border:none;border-radius:50%;width:32px;height:32px;font-size:20px;cursor:pointer;">&times;</button>
                    <button id="trolywp-chat-mode-sidebar" style="background:#eee;border:none;border-radius:6px;padding:0 12px;height:32px;cursor:pointer;">Sidebar</button>
                    <button id="trolywp-chat-mode-popup" style="background:#eee;border:none;border-radius:6px;padding:0 12px;height:32px;cursor:pointer;">Popup</button>
                    <button id="trolywp-chat-mode-fixed" style="background:#eee;border:none;border-radius:6px;padding:0 12px;height:32px;cursor:pointer;">Fixed</button>
                </div>
                <iframe id="trolywp-chat-iframe" src="' . esc_url($webhook_url) . '" style="width:100%;height:100%;border:none;"></iframe>
            </div>';
            echo '<script>
